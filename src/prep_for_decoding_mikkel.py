...
def fit_first_level_subject_per_trial(subject, bids_dir, mask, runs = [1, 2, 3, 4, 5, 6], space = "MNI152NLin2009cAsym"):
    ...
    for i in range(len(runs)):
        ...
        # create a design matrix with one regressor per trial
        dm = make_first_level_design_matrix(
            frame_times = events[i]["onset"], 
            events = events[i], 
            hrf_model = "glover", 
            drift_model = "cosine", 
            add_regs = confounds[i], 
            add_reg_names = confounds[i].columns.tolist(), 
            min_onset = 0,
            )
        ...
        # fit the model
        flm = first_level_model.fit(fprep_func_paths[i], design_matrices = dm)
...
if __name__ in "__main__":
    ...
    outpath = path / "data" / "decoding_new" 
    ...
    for subject in subjects:
        ...
        for i, flm in enumerate(flms):
            ...
            regressor_names = [name for name in regressor_names if "positive" in name or "negative" in name or "button_press"]
